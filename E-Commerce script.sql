...
select date_format(Order_Date, '%M -%y') as `Month`, Sum(d.Amount) as Monthly_Sales
from l_orders l inner join order_d d on l.`Order ID`= d.`Order ID`
group by `Month`;
...
select  l.CustomerName, round(sum(d.Amount)/count(l.CustomerName),2) as AOV
from l_orders l inner join order_d d on l.`Order Id`= d.`Order Id`
...
from t 
where total_sales > (select avg(amount) as avg_sales
                     from order_d)
